          ...
          {myGuide && (
            <Card className="border-primary/40 shadow-md">
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                {(() => {
                  const dailyLinks = myGuide.links.filter((l) => (l.cadence ?? "daily") === "daily");
                  const doneCount = dailyLinks.filter((l) => completed[l.path]).length;
                  const pct = dailyLinks.length ? Math.round((doneCount / dailyLinks.length) * 100) : 0;
                  return (
                    <div className="space-y-2 no-print">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-medium">{doneCount} / {dailyLinks.length} ({pct}%)</span>
                      </div>
