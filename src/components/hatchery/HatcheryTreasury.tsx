...
export default function HatcheryTreasury() {
  ...
  const { data: txns = [] } = useQuery({
    queryKey: ["hatchery_treasury_txns"],
    queryFn: async () =>
      (await supabase.from("hatchery_treasury_txns").select("*").order("txn_date", { ascending: false }).limit(2000)).data || [],
  });
  const { data: customers = [] } = useQuery({
    queryKey: ["hatch_customers_tr"],
    queryFn: async () => (await supabase.from("hatch_customers").select("id,name")).data || [],
  });
  const { data: batches = [] } = useQuery({
    queryKey: ["hatch_batches_tr"],
    queryFn: async () => (await supabase.from("hatch_batches").select("id,batch_number").order("receive_date", { ascending: false }).limit(500)).data || [],
  });
