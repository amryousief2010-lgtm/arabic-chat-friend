...
describe("Lovable remaining edge-function auth hardening", () => {
  ...
  it("user-owned mcp function is not regenerated by the Vite plugin (Lovable Publish)", () => {
    const mcp = fn("mcp");
    const viteConfig = readFileSync(resolve(process.cwd(), "vite.config.ts"), "utf8");
    expect(mcp).not.toMatch(/AUTO-GENERATED by @lovable\.dev\/mcp-js/);
    expect(mcp).toMatch(/Ownership taken/);
    expect(viteConfig).not.toMatch(/@lovable\.dev\/mcp-js\/stacks\/supabase\/vite/);
    expect(viteConfig).not.toMatch(/^\s*mcpPlugin\s*\(/m);
  });
