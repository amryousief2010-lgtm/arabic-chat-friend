...
console.log("TotalRows:", sheet.length);
...
for (let i=3; i<sheet.length; i++) {
  ...
  rows.push({rowNumber:i+1, external_id, customer_name, is_capital:c, batch_seq, receive_date,
    ...
    charge_total:charge, received_money:recv, remaining:rem, dedup_key, errors:received_eggs<=0?["zero"]:[]});
...
console.log("SUMMARY:", JSON.stringify({total:rows.length, cap, ext, totalEggs, capEggs, extEggs, totalDamaged, totalNet, totalChicks, capChicks, extChicks, totalCharge, totalReceived, totalRemaining}));
