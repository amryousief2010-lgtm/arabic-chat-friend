...
CREATE TABLE IF NOT EXISTS public.courier_daily_cash_deposit_lines (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  deposit_id UUID NOT NULL REFERENCES public.courier_daily_cash_deposits(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_ccd_lines_deposit ON public.courier_daily_cash_deposit_lines(deposit_id);
CREATE INDEX IF NOT EXISTS idx_ccd_lines_order ON public.courier_daily_cash_deposit_lines(order_id);

GRANT SELECT, INSERT ON public.courier_daily_cash_deposit_lines TO authenticated;
GRANT ALL ON public.courier_daily_cash_deposit_lines TO service_role;

ALTER TABLE public.courier_daily_cash_deposit_lines ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authorized users view deposit lines"
ON public.courier_daily_cash_deposit_lines FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "Service role manages deposit lines"
ON public.courier_daily_cash_deposit_lines FOR ALL
TO service_role
...
BEGIN
  ...
  SELECT * INTO v_existing FROM public.courier_daily_cash_deposits
   WHERE custody_id = p_custody_id AND deposit_date = p_day LIMIT 1;
  IF FOUND THEN
  ...
  LOOP
    IF r.update_status_marker = 'gift' OR r.collection_method = 'none' THEN
      INSERT INTO public.main_warehouse_treasury_txns
        (direction, category, amount, reference, notes, performed_by, courier_name, status, performed_at)
      VALUES ('in','courier_free_order_proof',0,'PROOF-FREE-'||r.order_number||'-'||v_day_str,
        'إثبات أوردر مجاني — '||r.order_number||' — '||COALESCE(r.total,0)::text,
        v_uid,v_courier_name,'posted',now()) ON CONFLICT DO NOTHING;
      v_proof_count := v_proof_count + 1;
    ELSIF r.collection_method = 'mixed_payment' THEN
      IF r.voda > 0 THEN
        INSERT INTO public.main_warehouse_treasury_txns
          (direction, category, amount, reference, notes, performed_by, courier_name, status, performed_at)
        VALUES ('in','courier_vodafone_proof',0,'PROOF-VODA-'||r.order_number||'-'||v_day_str,
          'إثبات فودافون كاش — '||r.order_number||' — '||r.voda::text,
          v_uid,v_courier_name,'posted',now()) ON CONFLICT DO NOTHING;
        v_proof_count := v_proof_count + 1;
      END IF;
      IF r.insta > 0 THEN
        INSERT INTO public.main_warehouse_treasury_txns
          (direction, category, amount, reference, notes, performed_by, courier_name, status, performed_at)
        VALUES ('in','courier_instapay_proof',0,'PROOF-INSTA-'||r.order_number||'-'||v_day_str,
          'إثبات إنستاباي — '||r.order_number||' — '||r.insta::text,
          v_uid,v_courier_name,'posted',now()) ON CONFLICT DO NOTHING;
        v_proof_count := v_proof_count + 1;
      END IF;
      IF r.bank > 0 THEN
        INSERT INTO public.main_warehouse_treasury_txns
          (direction, category, amount, reference, notes, performed_by, courier_name, status, performed_at)
        VALUES ('in','courier_bank_transfer_proof',0,'PROOF-BANK-'||r.order_number||'-'||v_day_str,
          'إثبات تحويل بنكي — '||r.order_number||' — '||r.bank::text,
          v_uid,v_courier_name,'posted',now()) ON CONFLICT DO NOTHING;
        v_proof_count := v_proof_count + 1;
      END IF;
    END IF;
  ...
  INSERT INTO public.courier_daily_cash_deposit_lines
    (deposit_id, order_id, order_number, customer_name, order_total, courier_cash_due,
     ...
     collection_method, status, update_status_marker)
