...
-- 1) has_approved_duplicate_order: only true if approval is not yet consumed
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE a.customer_id = p_customer_id
      ...
      AND a.resolved_order_id IS NULL
  );
$function$;

-- 2) mark_duplicate_order_approval_used: raise if already consumed
CREATE OR REPLACE FUNCTION public.mark_duplicate_order_approval_used(p_id uuid, p_order_id uuid)
...
BEGIN
  ...
  IF v_row.resolved_order_id IS NOT NULL AND v_row.resolved_order_id <> p_order_id THEN
    RAISE EXCEPTION 'APPROVAL_ALREADY_USED';
  END IF;
...
-- 3) check_duplicate_order_attempt: only surface approved existing_request if not yet consumed
...
BEGIN
  ...
  WHERE o.customer_id = p_customer_id
    ...
    AND public.order_items_signature(o.id) = v_item_sig
  LIMIT 1;
  ...
  -- Only consider approvals that haven't been consumed yet
  SELECT *
  INTO v_existing
