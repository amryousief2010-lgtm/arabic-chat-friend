...
DECLARE
  ...
  v_sl_inv_id UUID;
BEGIN
  SELECT id, destination_type, sale_no, destination_ref_id INTO v_sale
  FROM feed_sales WHERE id = OLD.sale_id;
  ...
  ELSIF v_sale.destination_type = 'slaughterhouse_feed_store' THEN
    SELECT id INTO v_sl_inv_id FROM slaughterhouse_feed_inventory WHERE feed_product_id = OLD.feed_product_id;
    IF v_sl_inv_id IS NOT NULL THEN
      INSERT INTO slaughterhouse_feed_movements(
        feed_id, movement_type, quantity_kg, unit_cost, total_cost,
        source_type, source_id, invoice_no, notes
      ) VALUES (
        v_sl_inv_id, 'reversal', -OLD.quantity, OLD.unit_price, -OLD.quantity * OLD.unit_price,
        'feed_factory_invoice_reversal', OLD.id, v_sale.sale_no,
        'عكس تلقائي لإلغاء بند فاتورة مصنع العلف رقم ' || COALESCE(v_sale.sale_no, '')
      );
    END IF;
