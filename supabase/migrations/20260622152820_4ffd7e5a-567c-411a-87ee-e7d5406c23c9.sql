...
BEGIN
  ...
  -- Fallback: if hatch_mortality_count is null/0, derive from transferred - chicks - unhatched.
  -- transferred falls back to fertile_eggs, then to eggs_in - infertile (excludes 'مستبعد').
  v_transferred := COALESCE(
    NULLIF(l.transferred_count,0),
    NULLIF(l.fertile_eggs,0),
    GREATEST(0, COALESCE(l.eggs_in,0) - v_infertile)
  );
  v_hatch_mort := COALESCE(
    NULLIF(l.hatch_mortality_count, 0),
    GREATEST(0, v_transferred - v_chicks - v_unhatched)
  );
...
-- Recompute all existing external-client hatchery invoices so the new
-- hatcher-mortality fallback rule is applied uniformly. No treasury / no payment
-- side-effects: the function only updates amount columns on the invoice row.
DO $$
DECLARE
  rec RECORD;
BEGIN
  FOR rec IN
    ...
    JOIN public.hatchery_client_invoices i ON i.id = l.invoice_id
    WHERE l.owner_type = 'external_client'
  LOOP
    PERFORM public.compute_hatchery_invoice(rec.lot_id);
  END LOOP;
END $$;
