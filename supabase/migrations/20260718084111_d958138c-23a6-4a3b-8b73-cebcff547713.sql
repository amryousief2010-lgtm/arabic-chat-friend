...
-- 1) Drop unique (custody_id, deposit_date) constraint to allow supplemental deposits per day
ALTER TABLE public.courier_daily_cash_deposits
  DROP CONSTRAINT IF EXISTS courier_daily_cash_deposits_custody_id_deposit_date_key;

-- 2) Recreate the RPC so it deposits only orders NOT already in a prior deposit for the same day
...
BEGIN
  ...
  SELECT courier_name INTO v_courier_name
    FROM public.courier_goods_custodies WHERE id = p_custody_id;
  IF v_courier_name IS NULL THEN RAISE EXCEPTION 'العهدة غير موجودة'; END IF;

  -- Collect order IDs already deposited previously for this custody/day
  SELECT COUNT(*), COALESCE(array_agg(x), '{}')
    INTO v_prior_count, v_already_ids
    FROM (
      SELECT UNNEST(order_ids) AS x
        FROM public.courier_daily_cash_deposits
       WHERE custody_id = p_custody_id AND deposit_date = p_day
    ) sub;

  -- Validate mixed_payment breakdown for orders that will be deposited (excluding already deposited)
  WITH day_asn AS (
    SELECT a.id AS assignment_id, a.order_id,
     ...
     WHERE a.custody_id = p_custody_id
       ...
       AND NOT (a.order_id = ANY(v_already_ids))
  ), validate AS (
    SELECT COUNT(*) FILTER (
      WHERE collection_method = 'mixed_payment'
        AND ABS(
          COALESCE(courier_cash_due,0)+COALESCE(deposit_amount,0)+COALESCE(vodafone_cash_amount,0)
          +COALESCE(instapay_amount,0)+COALESCE(bank_transfer_amount,0)
          +COALESCE(other_amount,0)+COALESCE(free_amount,0)-COALESCE(total,0)
        ) > 0.01
  ...
  WITH day_asn AS (
    ...
    UPDATE public.orders o
       SET status = 'delivered', payment_status = 'paid', collection_status = 'collected',
    ...
    ON CONFLICT (order_id) DO UPDATE
       SET courier_status = EXCLUDED.courier_status,
           ...
           last_updated_by = EXCLUDED.last_updated_by, updated_at = now()
    RETURNING order_id
  ), up_collections AS (
    ...
    SELECT id,
      ...
      FROM upd_orders
    ON CONFLICT (order_id) DO UPDATE
       SET amount_due = EXCLUDED.amount_due, amount_collected = EXCLUDED.amount_collected,
  ...
  SELECT COALESCE(SUM(CASE
           ...
           THEN CASE WHEN collection_method = 'mixed_payment' THEN COALESCE(courier_cash_due, 0)
                     ELSE COALESCE(total, 0) END
           ELSE 0 END), 0),
         COUNT(*),
  ...
  IF v_orders_count = 0 THEN
    IF v_prior_count > 0 THEN
      RAISE EXCEPTION 'لا توجد أوردرات جديدة لهذا اليوم — كل الأوردرات اتورّدت مسبقاً';
    ELSE
  ...
  v_reference := 'CASH-' || v_courier_name || '-' || v_day_str
                 || CASE WHEN v_prior_count > 0 THEN '-SUP' || (v_prior_count::text) ELSE '' END;
    ...
    VALUES
      ('in', 'courier_deposit', v_amount, v_reference,
       COALESCE(p_notes, '') || CASE WHEN COALESCE(p_notes,'')<>'' THEN ' — ' ELSE '' END
         || CASE WHEN v_prior_count > 0 THEN 'توريد إضافي لأوردرات ' ELSE 'توريد نقدية أوردرات ' END
         || 'يوم ' || v_day_label || ' — ' || v_courier_name || ' — ' || v_orders_count || ' أوردر',
  ...
  LOOP
    IF r.update_status_marker = 'gift' OR r.collection_method = 'none' THEN
      ...
      v_note := 'إثبات أوردر مجاني — ' || r.order_number
                ...
                || ' — قيمة الأوردر: ' || COALESCE(r.total,0)::text || ' — لا يوجد تحصيل نقدي';
      INSERT INTO public.main_warehouse_treasury_txns
        (direction, category, amount, reference, notes, performed_by, courier_name, status, performed_at)
  ...
  SELECT v_deposit_id, o.id, o.order_number, COALESCE(c.name, ''),
         COALESCE(o.total,0),
         ...
         COALESCE(o.vodafone_cash_amount,0), COALESCE(o.instapay_amount,0),
         COALESCE(o.bank_transfer_amount,0), COALESCE(o.free_amount,0),
         COALESCE(o.other_amount,0), COALESCE(o.deposit_amount,0),
  ...
  RETURN jsonb_build_object(
    'ok', true, 'deposit_id', v_deposit_id, 'treasury_txn_id', v_txn_id,
    'amount', v_amount, 'orders_count', v_orders_count,
    'proof_count', v_proof_count, 'lines_count', v_lines_count,
    'reference', v_reference,
    'supplemental', v_prior_count > 0
  );
END;
$function$;
