CREATE OR REPLACE FUNCTION public.marketing_dashboard_summary(p_from timestamp with time zone, p_to timestamp with time zone, p_include_top_products boolean DEFAULT false)
 RETURNS TABLE(total_orders bigint, total_sales numeric, delivered_orders bigint, delivered_sales numeric, cancelled_orders bigint, gift_orders bigint, gift_original_value numeric, avg_order_value numeric, approved_expenses numeric, pending_expenses numeric, total_expenses numeric, new_customers_count bigint, repeat_customers_count bigint, top_source jsonb, top_area jsonb, top_products_summary jsonb, date_from timestamp with time zone, date_to timestamp with time zone)
 LANGUAGE sql
 STABLE
...
WITH ranged_orders AS (
  ...
  FROM ranged_orders
  WHERE COALESCE(status, '') <> 'cancelled'
    AND (COALESCE(update_status_marker, '') = 'gift' OR COALESCE(collection_method, '') = 'none')
