...
DECLARE
  ...
  v_existing_total_batch_cost numeric := 0;
BEGIN
  SELECT total_cost, COALESCE(opening_cost_total,0), bird_count,
         COALESCE(dead_on_arrival,0), COALESCE(other_costs_loaded,0),
         COALESCE(manual_available_adjustment,0), COALESCE(total_batch_cost,0)
    INTO v_original_cost, v_opening_cost, v_bird_count, v_doa, v_other_cost, v_manual_adj, v_existing_total_batch_cost
  ...
  -- لو الدفعة عليها تسوية جرد يدوية وفيها تكلفة حالية محفوظة، نحافظ على قيمة الرصيد المتبقي ولا نعيد تحميل تكلفة الافتتاح بالكامل.
  IF v_manual_adj <> 0 AND v_existing_total_batch_cost > 0 THEN
    v_total := v_existing_total_batch_cost;
  ELSE
