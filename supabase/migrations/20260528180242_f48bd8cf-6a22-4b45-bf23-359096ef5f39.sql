...
BEGIN
  ...
  IF v_transfer.requested_by <> v_user_id AND NOT public.has_role(v_user_id, 'admin') THEN
    RETURN json_build_object('success', false, 'message', 'Only the requester or admin can cancel');
  END IF;

  -- only cancelable statuses
