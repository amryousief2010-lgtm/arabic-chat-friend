CREATE OR REPLACE FUNCTION public.update_transfer_request_quantities(
  p_transfer_id uuid,
  p_lines jsonb
)
RETURNS jsonb
...
BEGIN
  ...
  IF v_transfer.created_by IS DISTINCT FROM auth.uid()
     ...
     AND NOT public.has_role(auth.uid(), 'general_manager') THEN
  ...
  -- if no lines left, cancel the transfer
