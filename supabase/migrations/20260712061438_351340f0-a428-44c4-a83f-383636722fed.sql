...
BEGIN
  ...
  SELECT COALESCE(SUM(s.birds_count),0) INTO v_slaughtered
    FROM public.slaughter_batch_live_sources s
    JOIN public.slaughter_batches b ON b.id = s.slaughter_batch_id
   WHERE s.live_receipt_id = p_live_batch_id
