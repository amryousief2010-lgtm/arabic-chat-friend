...
BEGIN
  ...
  -- Lock custody row & validate it's open
  ...
  -- Idempotency: if a movement with same reference already exists, return it
  ...
  -- Verify orders are not already assigned (active)
  ...
  -- Iterate order items
  FOR v_item IN
    SELECT oi.id, oi.order_id, oi.product_id, oi.product_name, oi.quantity, oi.unit_price, oi.unit
  ...
  LOOP
    ...
    -- Resolve inventory item from main warehouse
    ...
    -- Insert inventory movement (posted -> trigger decrements stock)
    ...
    -- Custody line linked to the movement
    INSERT INTO public.courier_goods_custody_lines(
      ...
      p_custody_id, 'out', v_order.customer_id, v_order.customer_name, v_item.order_id,
  ...
  IF v_items_count = 0 THEN
    RAISE EXCEPTION 'لا يوجد أي صنف مرتبط بالمخزن الرئيسي. تعذر التجهيز. الأصناف: %', array_to_string(v_unresolved, '، ');
  END IF;

  -- Create courier order assignments
  INSERT INTO public.courier_order_assignments(custody_id, order_id, courier_name, assigned_at, assigned_by, status)
  SELECT p_custody_id, oid, v_courier, now(), v_user, 'with_courier'
...
GRANT EXECUTE ON FUNCTION public.approve_distribution_dispatch(uuid, uuid, uuid[], text) TO authenticated;
