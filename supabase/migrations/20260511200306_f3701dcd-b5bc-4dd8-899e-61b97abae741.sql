...
BEGIN
  ...
  IF NOT public.has_any_role(auth.uid(), ARRAY[
    ...
    'accountant'::app_role,'warehouse_supervisor'::app_role,'sales_moderator'::app_role,
  ...
  -- Monthly series for current year (up to today)
  WITH months AS (
  ...
  agg AS (
    ...
    FROM orders WHERE status <> 'cancelled' AND created_at::date >= v_year_start
    GROUP BY 1
  )
  ...
  agg AS (
    SELECT created_at::date AS d, SUM(total)::numeric AS sales, COUNT(*)::int AS orders
    FROM orders WHERE status <> 'cancelled' AND created_at::date BETWEEN v_month_start AND v_today
    GROUP BY 1
  )
  SELECT jsonb_agg(jsonb_build_object(
...
REVOKE EXECUTE ON FUNCTION public.get_production_dashboard(date, date) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.get_production_dashboard(date, date) TO authenticated;