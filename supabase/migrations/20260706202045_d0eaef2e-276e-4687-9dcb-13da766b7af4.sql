...
BEGIN
  ...
  FROM scored s
  WHERE
    -- Phone match is the primary signal — always a duplicate.
    s.match_phone
    -- Otherwise, only flag when BOTH name and address are highly similar
    -- on the same day. Items/shipping alone must never trigger approval.
