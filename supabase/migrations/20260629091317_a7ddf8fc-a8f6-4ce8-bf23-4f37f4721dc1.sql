...
BEGIN
  ...
  -- NEW: account for birds already sent to slaughter from this receipt
  SELECT COALESCE(SUM(birds_count),0)
    INTO v_slaughtered
    FROM public.slaughter_batch_live_sources
   WHERE live_receipt_id = p_live_batch_id;

  v_alive := GREATEST(v_bird_count - v_doa - v_mortality_count - v_slaughtered, 0);
...
-- Re-sync existing receipts so current balances reflect reality immediately
DO $$
DECLARE r record;
BEGIN
