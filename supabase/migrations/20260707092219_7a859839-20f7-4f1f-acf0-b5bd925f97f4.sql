...
-- 1) Add exact linkage column + uniqueness to prevent double-linking
ALTER TABLE public.sublocation_movements
  ADD COLUMN IF NOT EXISTS inventory_movement_id UUID REFERENCES public.inventory_movements(id) ON DELETE SET NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_sublocation_movements_inv_move
  ON public.sublocation_movements(inventory_movement_id)
  WHERE inventory_movement_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_inv_mov_item_performed
  ON public.inventory_movements(item_id, performed_at DESC);

-- 2) Rewrite trigger — match by item + not-already-linked + recent, with row locking
...
BEGIN
  ...
  -- Claim the most recent unlinked inventory_movement for THIS exact item
  -- (locks the row so a concurrent trigger cannot claim the same one)
  ...
  IF v_delta < 0 THEN
    ...
    LOOP
      ...
      -- Only link the first row to inventory_movement_id (uniqueness constraint)
      v_mov_id := NULL;
